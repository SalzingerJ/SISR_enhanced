...
def download_roi(item0, xyz, trg_filepath, pad_view=[[-0.25,-0.25],[0.25,0.25]], min_dim=1):
...
#load geotiff, transform into target webmercator data; band 3 == NIR
def convert_roi_nir(geotiff_path, xyz, calc_bandids=[3], dst_wh=(128,128), resampling_method=Resampling.nearest ):
...
def add_naip_nir(naip_coll, 
    ...
    t0 = time.time()
    nir_data = convert_roi_nir(tmp_geotiff_path, xyz)[:,:,3]
...
def main_callable(arg0, tqdm_ver=tqdm, retry_on_err=3, sleep_on_err=5.0):
    # parser setup
